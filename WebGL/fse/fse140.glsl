	if (preset == 140)
	{
		...
		if (near_yzx(data.normal, angle, d, ra) && !crit)
			res = vec3(0, 1, 1);
		crit = near_yzx(data.normal, 0., 0., d + ra - 0.1 * da);
		if (near_yzx(data.normal, angle, d, ra) && !crit)
			res = vec3(0.25, 0.25, 0.25);

		crit = near_yzx(data.normal, 0., 0., 3. * d / 2.);
		if (near_yzx(data.normal, angle, 3. * d / 2., 0.8 * da) && !crit)
			res = vec3(0, 1, 1);
		if (near_yzx(data.normal, angle, 3. * d / 2., 0.8 * da) && crit)
			res = vec3(0.25, 0.25, 0.25);
		...
		crit = inside_yz(data.normal, a_b + 0.65 * da, a_e - 0.65 * da);
		if (all(equal(res, vec3(1, 1, 1))) && !near_yzx(data.normal, 0., 0., d) && near_yzx(data.normal, 0., 0., 3. * d / 2.) && crit)
